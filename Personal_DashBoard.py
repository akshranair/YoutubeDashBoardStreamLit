...
#take the data of the past 12 months in all columns and then find median of each column
median_agg = df_agg_diff[df_agg_diff['Video publish time'] >= metric_date_12mo].select_dtypes(include = 'number').median()
...
if add_sidebar == 'Aggregate Metrics':
    st.write('Youtube Aggregate Data')
    ...
    metric_medians6mo = df_agg_metrics[df_agg_metrics['Video publish time'] >= metric_date_6mo].select_dtypes(include = 'number').median()
    metric_medians12mo = df_agg_metrics[df_agg_metrics['Video publish time'] >= metric_date_12mo].select_dtypes(include = 'number').median()
    ...
    df_agg_numeric_lst = df_agg_diff_final.select_dtypes(include="number").median().index.tolist()
...
if add_sidebar == 'Individual Video Analysis':
    ...
    agg_filtered = df_agg[df_agg['Video title']==videos_select]
    agg_sub_filtered = df_agg_sub[df_agg_sub['Video Title']==videos_select]
    ...
    mode='lines',
    name='20th percentiles', 
    line = dict(color='purple', dash='dash')))

    fig2.add_trace(go.Scatter(x=views_cumulative['days_published'], y=views_cumulative['20pct_views'],
    mode='lines',
    name='50th percentiles',
    ...
    fig.add_trace(go.Scatter(x=first_30['days_published'], y=first_30['Views'].cumsum(),
    mode='lines',
